...
enum {
  ...
  CREATE_TARGET_DIRS_OPTION
};

static char const *const update_type_string[] = {"all", "none", "none-fail",
                                                 "older", nullptr};
...
static struct option const long_options[] = {
    ...
    {"create-target-dirs", no_argument, nullptr, 'C'},
...
static bool do_move(char const *source, char const *dest, int dest_dirfd,
                    char const *dest_relname, const struct cp_options *x) {
  bool copy_into_self;
  bool rename_succeeded;
  bool ok = copy(source, dest, dest_dirfd, dest_relname, 0, x, &copy_into_self,
                 &rename_succeeded);
...
"),
          stdout);
    fputs(_("\
  -C, --create-target-dirs     create target directory if it doesn't exist\n\
"),
          stdout);
    fputs(_("\
...
/* Create parent directories of PATH, if needed and if the create_target_dirs
   flag is set. Return true on success, false on failure. */
static bool ensure_target_directory_exists(char const *path,
                                           bool create_target_dirs,
                                           bool verbose) {
  ...
  if (access(dir, F_OK) != 0) {
    ...
    if (make_dir_parents(dir, -1, NULL, NULL,
                         verbose ? GETMODE_VERBOSITY_YES
                                 : GETMODE_VERBOSITY_NO) != 0) {
      error(0, errno, _("cannot create directory %s"), quoteaf(dir));
      ok = false;
    } else if (verbose) {
      error(0, 0, _("created directory %s"), quoteaf(dir));
    }
    errno = saved_errno;
  }
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "bCfint:uvS:TZ", long_options,
                          nullptr)) != -1) {
    switch (c) {
    case 'b':
      make_backups = true;
    ...
    case 'u':
      ...
      if (optarg)
        x.update =
            XARGMATCH("--update", optarg, update_type_string, update_type);
      break;
    case 'v':
      x.verbose = true;
      break;
    ...
    case 'Z':
      ...
      if (selinux_enabled) {
        ...
        x.set_security_context = selabel_open(SELABEL_CTX_FILE, nullptr, 0);
  ...
  if (no_target_directory) {
    ...
    /* If the target directory is specified and --create-target-dirs is enabled,
       ensure the target directory exists before attempting to open it */
    if (create_target_dirs) {
      /* Try to create the target directory if it doesn't exist */
      if (access(target_directory, F_OK) != 0 && errno == ENOENT) {
        int saved_errno = errno;
        if (make_dir_parents(target_directory, -1, NULL, NULL,
                             x.verbose ? GETMODE_VERBOSITY_YES
                                       : GETMODE_VERBOSITY_NO) != 0) {
          error(EXIT_FAILURE, errno, _("cannot create target directory %s"),
                quoteaf(target_directory));
        }
        errno = saved_errno;
        if (x.verbose)
          error(0, 0, _("created directory %s"), quoteaf(target_directory));
      }
    }
    ...
    if (n_files == 2 && !x.exchange)
      x.rename_errno =
          (renameatu(AT_FDCWD, file[0], AT_FDCWD, lastfile, RENAME_NOREPLACE)
               ? errno
               : 0);
    if (x.rename_errno != 0) {
      ...
      if (target_dirfd_valid(fd)) {
        ...
        /* If create_target_dirs is enabled and the directory doesn't exist, try
         * to create it */
        if (create_target_dirs && err == ENOENT) {
          /* Create the directory with default permissions */
          if (make_dir_parents(lastfile, -1, NULL, NULL,
                               x.verbose ? GETMODE_VERBOSITY_YES
                                         : GETMODE_VERBOSITY_NO) == 0) {
            if (x.verbose)
              error(0, 0, _("created directory %s"), quoteaf(lastfile));

            /* Try opening again */
            fd = target_directory_operand(lastfile, &sb);
            if (target_dirfd_valid(fd)) {
              x.rename_errno = -1;
              target_dirfd = fd;
              target_directory = lastfile;
        ...
        if (err != 0 &&
            (2 < n_files || (O_PATHSEARCH == O_SEARCH && err == EACCES &&
  ...
  if (make_backups &&
      (x.exchange || x.update == UPDATE_NONE || x.update == UPDATE_NONE_FAIL)) {
    error(0, 0,
  ...
  if (target_directory) {
    ...
    /* If we're using create_target_dirs, ensure the parent directory exists */
    if (create_target_dirs) {
      ensure_target_directory_exists(file[1], create_target_dirs, x.verbose);
    }
