...
syscalls='sendfile'
other_syscalls='io_uring_setup io_uring_enter io_uring_register memfd_create splice tee vmsplice'
for s in $other_syscalls; do
  strace -qe "$s" true > /dev/null 2>&1 &&
    syscalls="$syscalls,$s"
done

no_zero_copy() { strace -f -o /dev/null -e inject=${syscalls}:error=ENOSYS "$@"; }
