...
# Make sure crc is correct for files larger than 32 bytes but <128 bytes (1 fold pclmul)
{
...
# Make sure crc is still handled correctly when next 65k buffer is read (>32 bytes more than 65k)
{
  seq 1 12780
} > in || framework_failure_
...
# Make sure crc is still handled correctly when next 65k buffer is read (>=128 bytes more than 65k)
{
  seq 1 12795
} > in || framework_failure_
