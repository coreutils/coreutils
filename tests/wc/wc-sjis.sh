...
# In coreutils v9.5 to 9.11 on glibc where SHIFT_JIS is available,
# wc indexed its 256-entry wc_isprint[]/wc_isspace[] tables with the
# decoded value, so reading past their end and crashed.

export LC_ALL=ja_JP.SHIFT_JIS

# Try to generate the locale if not available
# as it's generally not available by default.
...
test "$(locale charmap 2>/dev/null)" = SHIFT_JIS \
  || skip_ "SHIFT-JIS locale not available"
